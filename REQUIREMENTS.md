...
**需求细节**:
- 支持配置多个阿里云百炼平台的 AppID 和共享的 API Key
- **凭证格式**（统一使用共享 API Key 模式）：
  ```
  AppID1,AppID2,AppID3|SharedAPIKey
  ```
  - 使用 `,` 分隔多个 AppID
  - 使用 `|` 分隔 AppID 列表和共用的 API Key
  - 所有 Agent 共享同一个 API Key
  - 示例：`app-001,app-002,app-003|sk-xxxxxxxxxxxx`

- **凭证保护（可选）**：
  - 支持自定义前缀混淆，防止凭证明文泄露
  - 在凭证前添加前缀 `AGENT:`，系统自动识别并去除
  - 示例：`AGENT:app-001,app-002|sk-xxxxx`
  - 前缀可自定义，配置在代码常量中
  - 即添加了前缀，凭证也是明文存储在客户端，仅防止视觉泄露

- **输入便捷性**：
  - 支持 `id-` 自动转换为 `sk-`
  - 用户输入 `id-xxxxx` 时自动变为 `sk-xxxxx`
  - 便于快速输入，避免手动修改前缀

- **凭证配置规则**：
  - AppID 数量必须与 Agent 数量匹配
  - AppID 按顺序与 Agent 配置对应
  - API Key 应用到所有 Agent
  - 支持自动识别并去除混淆前缀
  - 格式验证和错误提示
...
| 2026-01-28 | v1.1 | 新增多 Agent 门户功能：<br>1. 支持共享 API Key 凭证配置（格式：AppID1,AppID2,AppID3|SharedAPIKey）<br>2. 新增 Agent 选择门户页面<br>3. 支持 Agent 切换和对话功能<br>4. 更新界面设计（门户页面+对话页面）<br>5. 更新技术架构和存储方案 | - |
| 2026-01-28 | v1.1.1 | 简化设计：移除配置管理页面，只保留凭证输入和 Agent 门户 | - |
